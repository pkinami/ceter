...
do $$ begin
  create type public.stock_status as enum ('in_stock', 'backorder', 'out_of_stock');
exception
  when duplicate_object then null;
end $$;
...
do $$ begin
  create type public.order_status as enum ('pending', 'processing', 'paid', 'fulfilled', 'cancelled');
exception
  when duplicate_object then null;
end $$;
...
do $$ begin
  create type public.quote_status as enum ('new', 'contacted', 'closed');
exception
  when duplicate_object then null;
end $$;
...
create table public.products (
  ...
  brand_id uuid references public.brands(id) on delete set null on update no action,
...
create table public.profiles (
  id uuid primary key references auth.users(id) on delete cascade on update no action,
  full_name text,
  phone text,
...
create table public.order_items (
  id uuid primary key default gen_random_uuid(),
  order_id uuid not null references public.orders(id) on delete cascade on update no action,
